...
For portability and reproducibility, there is a Docker containerisation possible using a ready made ```Dockerfile``` and ```environment.yml```. Below are instructions on how to run it.

For permissions reasons, the ```Dockerfile``` is not set up to pull the repository directly as it builds the image. Therefore, one must first download this repository to their local machine and then are free to build the Docker image from the ```Dockerfile```. In terms of dataset, the user should download the data from the external sources and place them in the ```Data/``` directory. This means that for the data generated by the MATLAB files, one should run those first before using the Docker container.

To run the code on the Docker container, one first has to build the image and run the container. This can be done as follows:

```bash
$ docker build -t project_24 .
$ docker run --rm -ti -v ./Data:/Project_24/Data project_24
```

The ```project_24``` is not a strict instruction, it can be set to any other name the user may prefer. This will build the image and run the container, also mounting the ```Data/``` directory as a volume so that the code can access the data on the local machine from the container.
...
- ChatGPT was used:
    ...
    - It was asked for recommendations on if any other library other than sklearn would have a SPCA implementation: It recommended SPAMS, by the same authors as the dictionary learning algorithm used in sklearn. This was not used in the end as the documentation was not clear on how to use it, and it seemed to require 2 complexity parameters, which would have made it harder to use than sklearn's implementation, on top of losing the quality of having not many hyperparameters to tune. It also recommended the mlxtend library, however, for this it seemed to simply lack an implementation of SPCA.
